...
- Origami result parsing is intentionally strict. The chat-completion prompt asks for exactly `true` or `false`, the runtime stores that text under `result`, and `_parse_overshoot_boolean` only accepts booleans or the strings `"true"` and `"false"` from that field. Legacy structured fallbacks such as `matches`, `match`, `value`, or `ok` should not drive fold progression.
...
The origami adapter should use the same strict boolean contract as the live runtime: prompt for exactly `true` or `false`, store the completion text under `result`, and parse only `result` booleans or `result` strings equal to `"true"` or `"false"`. Malformed structured responses should fail or be counted as invalid observations instead of being coerced through fallback keys.
