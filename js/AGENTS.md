# GlassKit JS Create Package Development
...
- `bin/create.js` is the plain JavaScript CLI entry point. Keep it dependency-free unless a new dependency clearly removes real complexity.
...
- `README.md` is package-facing and should remain clear that this first create package is WIP until it is ready to advertise from the repository root.
- `PUBLISHING.md` is the package runbook for npm publishing and the tag-triggered Trusted Publishing flow.

## Commands

- `npm ci`: install from the lockfile.
- `npm test`: rebuild the generated template and run the test suite.
- `npm run build`: refresh `dist/template/rokid-hello-world` from the canonical starter under `../skills/glasskit/assets/rokid-hello-world`.
...
npm releases are tag-triggered from the repository root workflow. `package.json` version and the pushed `npm-glasskit-ai-create-vX.Y.Z` tag must match.
