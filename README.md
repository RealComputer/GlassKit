...
Smart-glasses apps have constraints that coding agents are not used to handling: small HUDs, camera and microphone access, touchpad and voice inputs, WebRTC setup, vision AI pipelines, battery use, and wearer-facing UX. The GlassKit agent skill packages that context with reference patterns and a starter template, so agents can build more realistic glasses apps from the first pass.
