...
- `cd review-ui && npm install && npm run fix && npm run check`: install dependencies, apply lint fixes and formatting, and check the review UI after frontend changes. `uv build --no-sources --clear` rebuilds and embeds its production assets automatically.
