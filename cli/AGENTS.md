...
- `src/glasskit/eval/commands.py` and `process_adapters.py`: platform-native command parsing and display, process startup and teardown, NDJSON protocol transport, PNG sample serialization, request multiplexing and cancellation, stderr diagnostics, and command-adapter capability negotiation.
