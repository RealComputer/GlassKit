...
- The eval pipeline is app-agnostic. `expectations.py` discovers eval directories and case files, loads the eval config file, expands and filters case sample blocks, and resolves case videos. `schemas.py` owns Pydantic validation for YAML shape. `video.py` probes videos and streams requested frames from a single PyAV decoder per case. `adapters.py` imports adapter targets from `module:callable` or `file.py:callable`, distinguishes individual `evaluate` adapters from native `evaluate_many` batch adapters, and normalizes sync and async calls. `compare.py` evaluates JSON-like observations, and `runner.py` coordinates validation, on-demand frame consumption, bounded per-target concurrency, adapter execution, failure artifacts, deterministic JSON output, and quality gates. App-specific clients, prompts, parsers, workflow helpers, and secrets belong in adapters or the target app repo.
...
- `src/glasskit/eval/runner.py`: validation and run orchestration, on-demand frame cursor consumption, bounded individual-sample concurrency, target-bounded native batch dispatch, adapter timing, adapter lifecycle, reports, artifacts, and quality gates.
