...
- Generated review UI assets are no longer committed, superseding the plan's committed-output CI and release design. A Hatchling build hook now reinstalls the locked frontend dependencies and runs the Vite build for source checkouts, force-includes the ignored output in wheels and sdists, and verifies the embedded bundle when rebuilding a wheel from an sdist. CI exercises the hook from a clean checkout and rebuilds the resulting sdist with Node and npm excluded from `PATH`. This keeps runtime and sdist installation Node-free while making frontend source the only reviewed source of truth.
