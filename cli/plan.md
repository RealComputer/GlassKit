...
- Generated review UI assets are no longer committed, superseding the plan's committed-output CI and release design. A Hatchling build hook now runs the locked Vite build for source checkouts, force-includes the ignored output in wheels and sdists, and verifies the embedded bundle when rebuilding a wheel from an sdist. This keeps runtime and sdist installation Node-free while making frontend source the only reviewed source of truth.
