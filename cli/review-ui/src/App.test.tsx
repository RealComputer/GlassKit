...
describe('review application navigation and drafts', () => {
  ...
  it('keeps a backend-tolerated near-end timestamp valid on blur', async () => {
    const doc = caseDocument([
      target('near_end', [point('end', 10.01, '1')]),
    ])
    vi.stubGlobal(
      'fetch',
      vi.fn((input: RequestInfo | URL) => {
  ...
  it('blocks adding a point while the selected field draft is invalid', async () => {
    const doc = caseDocument([
      target('numeric_target', [point('first', 1, '1')]),
    ])
    vi.stubGlobal(
      'fetch',
      vi.fn((input: RequestInfo | URL) => {
        ...
        if (init?.method === 'PUT') {
          ...
          return Promise.resolve(
            response({
              ...
              targets: [
                target('numeric_target', payload.targets.numeric_target.points),
              ],
            }),
          )
        }
