...
function firstPointId(
  document: CaseDocument,
  targetId: string | null,
): string | null {
  return (
    document.targets.find((target) => target.id === targetId)?.points[0]?.id ??
    null
  )
}
  ...
  switch (action.type) {
    case 'SUITE_LOADED': {
      const selectedCaseId =
        ...
        action.suite.cases.some((item) => item.id === action.initialCaseId)
          ? action.initialCaseId
    ...
    case 'CASE_LOADED': {
      ...
      const targetId =
        preferred &&
        action.document.targets.some((target) => target.id === preferred)
          ? preferred
          : oldWorkspace &&
              action.document.targets.some(
              ...
              cases: state.suite.cases.map((summary) =>
                summary.id === action.caseId
                  ? {
                      ...
                      status:
                        action.document.status === 'blocked' ? 'blocked' : 'ready',
                    }
                  : summary,
              ),
            }
    ...
    case 'DISCARD_AND_LOAD': {
      const targetId =
        ...
        action.document.targets.some(
          ...
          ? state.selectedTargetId
          : firstTargetId(action.document)
      const target = action.document.targets.find(
...
export function isRepairComplete(workspace: CaseWorkspace): boolean {
  ...
  if (!document.editing_enabled || document.video?.duration_s === null || !document.video) return false
