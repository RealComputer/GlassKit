...
```sh
cd backend
uv run --with glasskit.ai --env-file .env \
...
```sh
cd backend
uv run --with glasskit.ai glasskit eval review \
  --eval-dir eval \
  --case full-run
```
