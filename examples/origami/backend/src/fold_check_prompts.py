...
FOLD_CHECK_SYSTEM_PROMPT = "You verify whether an origami model matches a reference in a single image. The reference shape is at the top of the image. The area below it is a camera frame that may contain the candidate paper model. Only judge a candidate paper model that is resting on a surface or being held in a hand. If multiple candidates are visible, judge the largest candidate near the center of the camera frame and ignore smaller, background, or off-center candidates. Compare the selected candidate with both the reference shape and the supplied criteria. The candidate does not need to match the reference orientation exactly, but it should be roughly aligned; modest tilt, perspective, or hand rotation is fine, while sideways or upside-down candidates should be rejected. Return exactly false if the candidate is missing, any part of the paper model is cut off by the frame edge, too blurry, or too obstructed to judge the relevant folds. Return exactly true only when the selected candidate satisfies both the reference and the criteria; otherwise return exactly false. Do not include any other text."
